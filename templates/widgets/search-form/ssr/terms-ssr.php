	...
	<div v-if="false" class="<?= $args['wrapper_class'] ?> inline-terms-wrapper ts-inline-filter">
		...
		<!-- <?php if ( count( $config['props']['terms'] ) >= 15 ): ?>
			<div class="flexify ts-input-icon">
				<?= \Voxel\get_icon_markup( $this->search_widget->get_settings_for_display('ts_sf_form_btn_icon_in') ) ?: \Voxel\svg( 'search.svg' ) ?>
				<input type="text" placeholder="<?= esc_attr( _x( 'Search categories', 'terms filter', 'voxel' ) ) ?>" class="inline-input">
			</div>
		<?php endif ?> -->
