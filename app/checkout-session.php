...
class Checkout_Session {
	...
	public function checkout() {
		if ( $this->is_using_price_id() ) {
			$args = $this->checkout_mode_price_id();
		} else {
			if ( $this->product_type->get_payment_mode() === 'subscription' ) {
	...
	protected function checkout_mode_price_id() {
		$customer = $this->customer->get_or_create_stripe_customer();
		...
		$intent_data_key = $pricing['payment_mode'] === 'subscription' ? 'subscription_data' : 'payment_intent_data';

		$args[ $intent_data_key ] = [
			'metadata' => [
				'voxel:payment_for' => 'vendor_product',
			],
		];

		if ( $intent_data_key === 'payment_intent_data' ) {
			$args[ $intent_data_key ]['capture_method'] = $this->product_type->config( 'settings.payments.capture_method', 'manual' );
		}
		...
		if ( $this->destination ) {
			if ( $this->product_type->config( 'checkout.on_behalf_of' ) ) {
				$args[ $intent_data_key ]['on_behalf_of'] = $this->destination->id;
			}

			$args[ $intent_data_key ]['application_fee_amount'] = $this->product_type->calculate_fee(
				$this->get_pricing_unit_amount()
			);

			$args[ $intent_data_key ]['transfer_data'] = [
				'destination' => $this->destination->id,
			];
		}
